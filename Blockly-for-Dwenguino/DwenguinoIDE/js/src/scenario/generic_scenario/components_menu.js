...
class DwenguinoSimulationRobotComponentsMenu {
   ...
   * has to be called when the scenario is created.
   * @param {GenericScenario} genericScenario - The social robot scenario to which the social robot components are added.
   */
  setupEnvironment(genericScenario) {
    this.GenericScenario = genericScenario;
     ...
     * // should actually already be handled by the genericScenario which is used here.
     */
    this.initMenu();
  ...
  addRobotComponent(id) {
    ...
    this.genericScenario.addComponent(type);
  ...
  removeRobotComponent(id) {
    let type = this.extractType(id);
    this.genericScenario.removeComponentbyType(type);
